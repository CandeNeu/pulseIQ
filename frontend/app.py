...
Generate the recommendations as JSON."""
    ...
    for attempt in range(3):
        try:
            ...
            try:
                error_details = (
                    e.response.json().get("error", {}).get("message", "Unknown API error")
                )
            except Exception:
...
with st.container(horizontal=True, vertical_alignment="center"):
    ...
    with st.container(horizontal=True, horizontal_alignment="right"):
        if st.session_state.patient_ref:
            st.markdown(
                f":gray[Ref] `{st.session_state.patient_ref}`", width="content"
            )
        st.markdown(
            f":gray[Session] `{st.session_state.session_id}`", width="content"
        )
        st.markdown(
...
with st.sidebar:
    ...
    st.caption(
        "Colours follow IEC 60601-1-8 and bedside-monitor parameter convention."
    )
...
if step == 1:
    ...
    if height < 1.2 or height > 2.4:
        validation_issues.append(f"Height {height:.2f}m is outside typical adult range (1.2–2.4m)")
    if weight < 30 or weight > 200:
        validation_issues.append(f"Weight {weight:.1f}kg is outside typical adult range (30–200kg)")
    if age < 18:
        validation_issues.append("Age is below 18 years — this screening is designed for adults")
...
# ==========================================================================
    ...
    def render_idle(message):
        ...
        with trace_col:
            ...
            flat = pd.DataFrame(
                {"sample": np.arange(120), "amplitude": np.zeros(120)}
            )
            st.altair_chart(
                alt.Chart(flat)
    ...
    if mode == "Upload recording":
        ...
        def live_guidance(status, estimate, state, captured):
            ...
            if captured:
                return "Reading captured — keep still for a steadier value, or run the assessment.", "green"
            if status.hint == "empty":
                return "Waiting for camera frames…", "gray"
            if status.hint == "uncovered":
                return "Lens not covered — press a fingertip flat over the camera lens.", "yellow"
            if status.hint == "dark":
                return "Too dark — turn towards a lamp or window so light passes through the fingertip.", "yellow"
            if status.hint == "saturated":
                return "Sensor saturated — ease the finger pressure or move away from the light.", "yellow"
            if estimate is None:
                return f"Acquiring — hold still, first reading after {MIN_WINDOW_S:.0f} s.", "blue"
            return {
                "acquiring": (f"Hold still — accepted once the reading holds steady over {WINDOW_S:.0f} s.", "blue"),
                "stable": ("Steady — keep the finger in place while the window fills.", "blue"),
                "holding": ("Signal lost — holding the last value. Keep the finger still and fully over the lens.", "yellow"),
                "searching": ("No reliable pulse — reposition the fingertip and hold still.", "red"),
            }[state.status]
        ...
        def live_monitor():
            ...
            if captured:
                store_reading(
                    analysis_record(estimate, state.bpm, estimate.quality, "Live", "Stable")
                )
            ...
            if estimate is None:
                ...
                badge = ("Captured", "green") if captured else STATUS_BADGES[state.status]
                render_trace(estimate, "Live", state.bpm, estimate.quality, status=badge)
...
# ==========================================================================
    ...
    if elevated_count == 0:
        ...
        risk_summary = (
            f"⚠️ **You have elevated risk for {condition}** — clinical follow-up advised"
        )
    ...
    def risk_panel(column, title, icon, probability):
        ...
        with column:
            ...
            if elevated:
                ...
                st.badge("Within expected range", icon=":material/check:", color="green")
    ...
    with st.container(horizontal=True):
        ...
        with st.container(horizontal=True, horizontal_alignment="right"):
            ...
            if st.button(
                "New session", type="primary", icon=":material/restart_alt:"
            ):
